...
export class Parser {
	...
	async parse_for_tasks(){
		...
		for (let day_no = 1; day_no <= 7; day_no ++){
			let yesterday_file  = normalizePath(this.settings.CustomFolder + `/` + moment().subtract(day_no, "days").format(this.settings.DateFormat) + `-` + this.settings.CustomFile + `.md`);
			...
			//	let file_array : any
				...
				if (!file_contents){
					new Notice("No Tasks in previous note: " + moment().subtract(day_no, "days").format(this.settings.DateFormat))
				}else{
